...
from models import LeNet_FashionMNIST, CNN_Cifar10, CNN_Cifar100, Model_adults, Model_purchase, TextCNN, ViT_Cifar10, AlexNet
import numpy as np
...
def model_init(args):
    if args.data_name == 'mnist':
        ...
        # model = AlexNet.AlexNet(args.num_classes, 3)
#         model = ViT_Cifar10.Model(
#     image_size = 32,
#     patch_size = 4,
#     num_classes = 10,
#     dim = 512,
        ...
        if args.data_name == 'text':
            ...
            for batch in test_loader:
                ...
                for idx, label in enumerate(targets):
                    ...
                    label_data_dict[label].append(
                        (torch.tensor(data[idx]), torch.tensor(at[idx]), torch.tensor(label)))
        else:
    ...
    for user in range(args.num_user):
        ...
        if args.data_name == 'text':
            for batch in client_loaders[user]:
                ...
                dataset_image.extend(data)
                dataset_at.extend(at)
                dataset_label.extend(targets)
            data_x = []
            data_at = []
            data_y = []
...
def backdoor_attack(args, client_loaders):
    for client in args.forget_client_idx:
        dataset_x = []
        dataset_y = []
        dataset_at = [] if args.data_name == 'text' else None
        for idx, batch in enumerate(client_loaders[client]):
            if args.data_name == 'text':
                data, at, target = batch
            else:
                data, target = batch
            if idx <= 1:  
                if args.data_name == 'text':
                    ...
                    dataset_at.extend(at.cpu().detach().numpy())
        ...
        if args.data_name == 'text':
            ...
            dataset_x = torch.Tensor(dataset_x).type(torch.float32)
            dataset_at = torch.Tensor(dataset_at).type(torch.float32)
            dataset_y = torch.Tensor(dataset_y).type(torch.int64)
            dataset = [(x, at, y) for x, at, y in zip(dataset_x, dataset_at, dataset_y)]
            client_loaders[client] = torch.utils.data.DataLoader(dataset, batch_size=args.local_batch_size, shuffle=False)
        else:
            dataset_x = torch.Tensor(dataset_x).type(torch.float32)
            dataset_y = torch.Tensor(dataset_y).type(torch.int64)
            dataset = [(x, y) for x, y in zip(dataset_x, dataset_y)]
    ...
    for i in range(args.num_user):
        ...
        for batch_idx, data in enumerate(data_loader):
            ...
            if args.data_name == 'text':
                ...
                test_x.extend(outputs.logits.cpu().detach())
                test_x_user[i].extend(outputs.logits.cpu().detach())
            else:
                images, labels = data[0].to(args.device), data[1].to(args.device)
...
def train_shadow_model(args, case, model, proxy_client_loaders_bk, proxy_client_loaders, proxy_test_loaders):
    ...
    for i in range(args.n_shadow):
        ...
        elif args.paradigm == 'fedau':
            if args.forget_paradigm == 'class':
                shadow_unlearning_model = case.train(proxy_client_loaders, proxy_test_loaders)
            elif args.forget_paradigm == 'client':
                shadow_unlearning_model = case.train(proxy_client_loaders_bk, proxy_test_loaders)
            elif args.forget_paradigm == 'sample':
                shadow_unlearning_model = case.train(proxy_client_loaders, proxy_test_loaders)
        ...
        for user in range(args.num_user):
            ...
            for batch_idx, data in enumerate(attack_loader):
                ...
                if args.data_name == 'text':
                    ...
                    attack_x_train.extend(outputs.logits.cpu().detach())
                    attack_x_train_user[user].extend(outputs)
                else:
                    images, labels = data[0].to(args.device), data[1].to(args.device)
        ...
        if args.forget_paradigm == 'class':
            ...
            for data in attack_test_loader:
                if args.data_name == 'text':
                    ...
                    outputs = shadow_unlearning_model(images)
                    attack_x_train.extend(outputs.cpu().detach())
                labels = labels.cpu()  # 将 labels 移动到 CPU
