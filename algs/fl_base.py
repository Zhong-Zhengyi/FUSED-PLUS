...
class Base(object):
    ...
    def FL_Retrain(self, init_global_model,  client_all_loaders, test_loaders, FL_params):
        ...
        if FL_params.forget_paradigm == 'client':
            ...
            fnum = len(self.args.forget_class_idx)
            ...
            fnum = self.args.backdoor_frac
        df['Comsume_time'] = consume_time

        if self.args.cut_sample == 1.0:
            if self.args.save_normal_result:
                df.to_csv(
                './results/{}/Acc_loss_retrain_{}_data_{}_distri_{}_fnum_{}.csv'.format(self.args.forget_paradigm, self.args.forget_paradigm, self.args.data_name, self.args.alpha, fnum))
            ...
            if self.args.data_name == 'text':
                for batch in data_loader:
                    ...
                    if isinstance(batch, (list, tuple)):
                        ...
                        input_ids = batch['input_ids'].to(FL_params.device)
                        attention_mask = batch['attention_mask'].to(FL_params.device)
                        labels = batch['labels'].to(FL_params.device)
            ...
            if self.args.data_name == 'text':
                ...
                for batch_idx, batch in enumerate(test_loader):
                    ...
                    if isinstance(batch, (list, tuple)):
                        ...
                        input_ids = batch['input_ids'].to(FL_params.device)
                        attention_mask = batch['attention_mask'].to(FL_params.device)
                        target = batch['labels'].to(FL_params.device)
                    input_ids = input_ids.long()
                    input_ids = input_ids.to(FL_params.device)
                    outputs = model(input_ids, attention_mask)
                    logits = outputs.logits
