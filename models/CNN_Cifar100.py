...
class Model_UL(MyModel):
    def __init__(self, config):
        ...
        # 添加遗忘层
        self.classifier_ul = nn.Linear(num_ftrs, self.num_classes)

    def forward(self, x):
        # 获取ResNet18的特征提取部分（不包括最后的全连接层）
        ...
        # 获取原始输出
        original_output = self.model.fc(features)
        # 获取遗忘层输出
        forget_output = self.classifier_ul(features)
        # 合并两个输出
        combined_output = torch.cat((original_output, forget_output), dim=1)
